...
function buildQueueItem(evidence, facility, analysis = null, job = null) {
  ...
  // Accepted evidence with no outstanding AI review is settled: it should not
  // sit in the queue just because it is unmatched or maps to a high-priority
  // rule. Suspicious scans, expirations, and rejections still queue above.
  const settled = evidence.status === "accepted" && !(analysis?.needsHumanReview && !analysis?.humanReviewed);
  if (!suggestedRuleId && !settled) categories.push("unmatched");
  if (["critical", "high"].includes(rule?.priority) && !settled) categories.push("high_priority_impact");
