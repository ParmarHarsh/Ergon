...
function recommendedAction(ruleItem, missingTypes, status) {
  ...
  return `Collect or log evidence for: ${missingTypes.map((type) => type.replaceAll("_", " ")).join(", ")}.`;
}
...
export function computeReadinessScore(gapRows) {
  ...
  const deductions = [
    ["critical", criticalMissing, 25, "critical obligation missing required evidence", "critical obligations missing required evidence"],
    ["high", highMissing, 15, "high-priority obligation missing required evidence", "high-priority obligations missing required evidence"],
    ["medium", mediumMissing, 8, "medium-priority obligation missing required evidence", "medium-priority obligations missing required evidence"],
    ["expired", expiredEvidence, 10, "expired evidence item", "expired evidence items"],
    ["rejected", rejectedEvidence, 5, "rejected evidence item", "rejected evidence items"]
  ];
  ...
  const scoreExplanation = deductions
    ...
    .map(([, count, points, singular, plural]) => `-${points * count} pts: ${count} ${count === 1 ? singular : plural}`);
